...
class ObjectRangeFilter(object):
    ...
    def __call__(self, input_dict):
        ...
        gt_bboxes_3d = input_dict['gt_bboxes_3d']
        gt_labels_3d = input_dict['gt_labels_3d']
        mask = gt_bboxes_3d.in_range_bev(bev_range)  #---------根据中心点xy在BEV视角下---
        gt_bboxes_3d = gt_bboxes_3d[mask]  #--在base_box3d的__get_item__这里可以取数，根据索引或者torch.booltensor值-
        # mask is a torch tensor but gt_labels_3d is still numpy array
        # using mask to index gt_labels_3d will cause bug when
        # len(gt_labels_3d) == 1, where mask=1 will be interpreted
        # as gt_labels_3d[1] and cause out of index error，这里需要时在解决
        #-------这里gt_labels_3d为numpy类型，注意索引问题--------
        gt_labels_3d = gt_labels_3d[mask.numpy().astype(np.bool)]

        # limit rad to [-pi, pi]
        #---------这里将输入的角度限制在-pi到pi之间，因为输入时进行了转换--------
        #---------原始的朝前为0，左为正，但是需要转成LiDARInstance3DBoxes,在pkl中已经转换，
        #-------- -rots - np.pi / 2，导致第三象限有值小于-pi--------------------------------
...
#----------根据类别名字过滤掉真值中不在所需类别的目标bbox及label--------------
@PIPELINES.register_module()
