...
class LoadMultiViewImageFromFiles(object):
    ...
    def __call__(self, results):
        ...
        # ----------在这里先scale掉前后视，但是lidar2img也要对应改-------------
        img = np.stack(
        ...
        results['img'] = [img[..., i] for i in range(img.shape[-1])] #--按顺序六个图像，（900,1600,3）--
        results['img_shape'] = img.shape  #---（900,1600,3,6）下面都是赋初始值字段----
...
#----------------这里整理anno结构，生成'gt_bboxes_3d''gt_labels_3d'等，
#----------------并在bbox3d_fields加入'gt_bboxes_3d'字段----------
@PIPELINES.register_module()
