...
from newscenes_devkit.eval.common.data_classes import MetricData, EvalBox
...
class DetectionMetrics:
    ...
    def no_score(self) -> float:
        """
        Compute the NewScenes Overall Score (NOS, weighted sum of the individual scores).
        :return: The NOS.
        """
        # Summarize.
...
class DetectionBox(EvalBox):
    ...
    def deserialize(cls, content: dict):
        ...
        return cls(sample_token=content['sample_token'],
                   ...
                   ego_translation=tuple(content['translation']), #---这里就是自车坐标系，因此直接与translation一致
                  
                   num_pts=-1 if 'num_pts' not in content else int(content['num_pts']),
                   detection_name=content['detection_name'],
                   detection_score=-1.0 if 'detection_score' not in content else float(content['detection_score']),
                   attribute_name='' if 'attribute_name' not in content else content['attribute_name'],#----没有属性标签
                   visibility = 1 if 'visibility' not in content else content['visibility']) #----加入图像可视标签
