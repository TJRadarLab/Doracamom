            ...
            if box_cls == DetectionBox:
                ...
                sample_boxes.append(
                    box_cls(
                        ...
                        ego_translation = tuple(box.center.tolist()),#--本身在ego/lidar下
                        num_pts=-1, #--暂时不用这个
                        detection_name=detection_name,
                        detection_score=-1.0,  # GT samples do not have a score.
                        attribute_name='', #--不用这一维度
...
#-----------加入每个bbox离ego的xyz距离，这里原始的都在ego/lidar下不需要加入----------------
...
def filter_eval_boxes(newsc: NewScenes,
    ...
    for ind, sample_token in enumerate(eval_boxes.sample_tokens):
        ...
        # Filter on distance first.首先根据每类的距离过滤,根据标注范围过滤
