...
#-----------------------------读取对应json文件----------------
    ...
    def __load_table__(self, table_name) -> dict:
        ...
        return table
#-----------------------------------------------
    #-----------token和按顺序的index之间的映射----------------
    #--------这里对于imu和pose需要重新按场景_token来定义------
...
#----------这里pose和imudata也要对应修改，加入scene_token----------------
    def get_annotation_box(self, sample_token: str) -> Box:
        ...
        ego_pose = self.get('ego_pose', sync['ego_pose']['lidar_top_compensation'],scene_token)['pose'] #--16位pose
        ...
        for box in record:
            box_center = [box['center']['x'], box['center']['y'], box['center']['z']] #--中心点
            box_size = [box['size']['y'], box['size']['x'], box['size']['z']] #--wlh这里是宽长高
            box_orientation = Quaternion(axis=[0, 0, 1], radians=box['rotation']['z']) #--这里是z轴旋转角度
            box_visibility = box['visibility']
            box_name = box['category']
            box_id = box['id']
            #---------加入目标的速度信息，并且转到自车坐标系下----------------
            box_velocity_global = box_velocity_dict[box_id] #---全局速度
            #---速度转换到ego坐标系下只有旋转没有平移--
...
#---------计算目标的速度，这里返回的是全局坐标系下的vxvyvz速度-----
#---------返回一个有序字典，即{id：[vx,vy,vz]}，id是目标的id，vxvyvz是目标全局的速度，按标注的顺序排--
    def box_velocity(self, sample_token: str, max_time_diff: float = 1.5) -> np.ndarray:
        ...
        for track_id, center_xyz in current_anno_center.items():
            ...
            if track_id not in prev_anno_center and track_id not in next_anno_center:
                ...
                continue
            #-------如果在前一帧有这个目标后一帧没有，就用当前帧和前一帧计算速度-------
             ...
             #-------如果在前一帧没有这个目标后一帧有，就用后一帧和当前帧计算速度-------
    ...
    def transform_anno_center_to_global(self, sample_token: str) -> dict:
        ...
        box_id = [] #---id号
        boxes_center_ego = [] #---自车坐标系下的中心点
        for box in current_anno:
            box_center = [box['center']['x'], box['center']['y'], box['center']['z']] #--中心点
            boxes_center_ego.append(box_center)
            box_id.append(box['id'])
        boxes_center_ego = np.array(boxes_center_ego)
        boxes_center_global = transform_points(boxes_center_ego, current_pose) #---转换到全局坐标系下
        anno_global_center = OrderedDict((id_val, center) for id_val, center in zip(box_id, boxes_center_global)) #---id和中心点对应
...
#             #------------这里对bbox进行了坐标变换，将其转换到了传感器坐标系下，原始标注在全局坐标下----------------
#         box_list = []
#         for box in boxes:
...
#         # Retrieve sensor & pose records
#         sd_record = self.get('sample_data', sample_data_token)
#         curr_sample_record = self.get('sample', sd_record['sample_token'])  #--找到sample_data对应的sample，按时间会找最近的，但不一定是关键帧----

#         if curr_sample_record['prev'] == "" or sd_record['is_key_frame']: #--------是关键帧直接取标注------------
#             # If no previous annotations available, or if sample_data is keyframe just return the current ones.
#             boxes = list(map(self.get_box, curr_sample_record['anns']))

#         else: #------------不是关键帧找之前的关键帧，然后插值------------
...
class NuScenesExplorer:
    ...
    def render_sample_data(self,
        ...
        if sensor_modality in ['lidar', 'radar']:
            ...
            if sensor_modality == 'lidar':
                ...
                velocities = pc.points[8:10, :]  #---补偿的速度---
                #---------这里速度进行了坐标旋转转换，从radar转到自车，从自车转到lidar-----
    ...
    #------在激光雷达和图像下绘制实例，一般是连续几帧实例找离车最近的绘制-------
...
if __name__ == '__main__':
    ...
    # nusc.render_sample_data(sample['data']['CAM_BACK'], with_anns=True)
    ...
    # nusc.render_sample_data(sample['data']['LIDAR_TOP'], nsweeps=5, underlay_map=True, use_flat_vehicle_coordinates=True)
    # nusc.render_sample_data(sample['data']['LIDAR_TOP'], nsweeps=5, underlay_map=True, use_flat_vehicle_coordinates=True, out_path='/tmp/figure.png')
    # nusc.render_annotation(sample['anns'][20], underlay_map=True)
    # nusc.render_annotation(sample['anns'][20], underlay_map=True, out_path='/tmp/figure.png')
    # nusc.render_instance(nusc.instance[0]['token'])
    # nusc.render_scene(my_scene['token'])
    # nusc.render_scene_channel(my_scene['token'], channel='CAM_FRONT')
    # nusc.render_scene_channel(my_scene['token'], channel='CAM_FRONT', out_path='/tmp/figure.png')
    # nusc.render_egoposes_on_map('singapore-onenorth', scene_tokens=[my_scene['token']])
    # nusc.render_scene_channel_lidarseg(my_scene['token'], channel='CAM_FRONT', out_folder='/tmp', filter_lidarseg_labels=[0, 1, 10], render_mode='image')
    # nusc.render_scene_lidarseg(my_scene['token'], out_path='/tmp/figure.avi', filter_lidarseg_labels=[0, 1, 10])
    # print("Done")