...
#---根据pipeline中顺序进行数据读取增强组装等，生成img_metas,gt_bboxes_3d,gt_labels_3d,img四个DC----
    ...
    #---------等效于直接4*4求逆矩阵，这里的平移旋转外参在pkl文件时已经是考虑了不同时刻egopose---------------
        ...
        if not self.test_mode:
            #---这里调用newscenesdataset中的get_ann_info，input_dict加入anno，包括gt_lables,gt_bboxes,gt_names，根据valid_flag过滤目标---
        ...
        can_bus[3:7] = input_dict['ego2global_rotation'] #---这里直接赋值Quaternion给array有问题--
        #------这里得到全局下自车的yaw-------
