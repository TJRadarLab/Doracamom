...
#         radar_points = clean_radar.tensor.numpy()
#         lidar_points_mask = results['points'].in_range_3d(self.pc_range)
#         lidar_points = results['points'][lidar_points_mask].tensor.numpy()
#         # 绘制散点图
#         ax.scatter(radar_points[:, 0], radar_points[:, 1], color='blue', s=20, label='Radar Points')  # 设置颜色为蓝色，点的大小为30
#         ax.scatter(lidar_points[:, 0], lidar_points[:, 1], color='red', s=0.2, alpha=0.5, label='Lidar Points')  # 设置颜色为红色，点的大小为10
#         ax.set_xlabel('X')
#         ax.set_ylabel('Y')
#         ax.set_title('Scatter Plot')
#         ax.legend()  # 显示图例
#         # 保存图像
#         plt.savefig('/mnt/zhenglianqing/bevformer_noted/debug_some_imgresult/scatter_plot.png',bbox_inches='tight', pad_inches=0.1, dpi=300)
# #-----------------------------------------------------
...
#             lidar_points_mask = results['points'].in_range_3d(self.pc_range)
#             lidar_points = results['points'][lidar_points_mask].tensor.numpy()
#             img_lidar = project_pts_on_img(lidar_points, results['img'][i], results['lidar2img'][i])
#             cv2.imwrite(f'/mnt/zhenglianqing/bevformer_noted/debug_some_imgresult/lidar_to_img_{i}.jpg', img_lidar)
#             radar_points = clean_radar.tensor.numpy()
#             img_radar = project_pts_on_img(radar_points, results['img'][i], results['lidar2img'][i],thickness=10)
#             cv2.imwrite(f'/mnt/zhenglianqing/bevformer_noted/debug_some_imgresult/radar_to_img_{i}.jpg', img_radar)
# #--------------------------------------------------------------------
