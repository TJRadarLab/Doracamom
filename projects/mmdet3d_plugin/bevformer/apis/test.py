...
def custom_multi_gpu_test(model, data_loader, tmpdir=None, gpu_collect=False):
    ...
    rank, world_size = get_dist_info() #---world size指进程总数，在这里就是我们使用的卡数；rank指进程序号,0主进程
    if rank == 0:
        prog_bar = mmcv.ProgressBar(len(dataset))  #---0/6019
    time.sleep(2)  # This line can prevent deadlock problem in some cases.
    have_mask = False
    for i, data in enumerate(data_loader):  #---取数----
        with torch.no_grad():
            #---result=[{'pts_bbox':{'boxes_3d':LiDARInstance3DBoxes,'scores_3d':tensor,'labels_3d':tensor}}]
    ...
    # collect results from all ranks从所有进程中集中结果，2卡的话，dataset长度100则每个50----
    if gpu_collect:
