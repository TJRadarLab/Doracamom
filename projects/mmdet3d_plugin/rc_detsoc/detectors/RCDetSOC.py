...
class RCDetSOC(MVXTwoStageDetector):
    """4DRC-DetSOC.
    Args:
    ...
    def forward_train(self,
        ...
        prev_img = img[:, :-1, ...] #--torch.Size([1, 2, 6, 3, 544, 960])
        img = img[:, -1, ...] #--torch.Size([1, 6, 3, 544, 960])
        #------点云序列--------
        prev_points = points[:-1] #--[[tensor],[tensor]]
        points = points[-1]
        #---------------------
...
#                   'prev_bev_radar':torch.Size([1, 2, 384, 160, 240])}
    ...
    def forward_test(self, img_metas,
        ...
        for var, name in [(img_metas, 'img_metas')]:
            if not isinstance(var, list):
                raise TypeError('{} must be a list, but got {}'.format(
                    name, type(var)))
        img = [img] if img is None else img #--torch.Size([1, 6, 3, 544, 960])
        ...
        while len(self.prev_frame_info["prev_bev"]['prev_bev_img']) >= self.queue_length:
