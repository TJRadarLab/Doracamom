...
class FRPN_OCC(BaseModule):
    ...
    def __init__(
        ...
        super(FRPN_OCC, self).__init__()
        self.mask_net = nn.Sequential(
            nn.Conv3d(in_channels, in_channels//2, kernel_size=3, padding=1, stride=1),
            nn.BatchNorm3d(in_channels//2),
            nn.ReLU(),
            nn.Conv3d(in_channels//2, 1, kernel_size=3, padding=1, stride=1),
            )
    ...
    def get_occ_mask_loss(self, gt_occ_mask, pred_occ_mask):
        assert gt_occ_mask.shape == pred_occ_mask.shape
        gt_occ_mask = gt_occ_mask.permute(0,4,3,2,1)
        pred_occ_mask = pred_occ_mask.permute(0,4,3,2,1)
        bs, _, occ_z, occ_h, occ_w = gt_occ_mask.shape
        b = gt_occ_mask.reshape(bs, occ_z * occ_h * occ_w).permute(1, 0).to(torch.float)
        a = pred_occ_mask.reshape(bs, occ_z * occ_h * occ_w).permute(1, 0)
        mask_ce_loss = self.ce_loss(a, b)*self.loss_weight
        mask_dc_loss = self.dice_loss(pred_occ_mask.reshape(bs, -1), gt_occ_mask.reshape(bs, -1))*self.loss_weight
        return dict(occ_mask_ce_loss=mask_ce_loss, 
                    occ_mask_dc_loss=mask_dc_loss)
