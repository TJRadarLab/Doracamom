...
class OccupancyEncoderV1(TransformerLayerSequence):
    ...
    def __init__(self, *args, pc_range=None, num_points_in_pillar=4, return_intermediate=False, dataset_type='nuscenes', ego=False,
        ...
        super(OccupancyEncoderV1, self).__init__(*args, **kwargs)
    ...
    def get_reference_points(self, H, W, Z=8, num_points_in_pillar=4, dim='3d', bs=1, device='cuda', dtype=torch.float):
        ...
        #---按比例的,0.5就是格子中心---高度是几就采几个点，保证每个格子一一对应----
        if dim == '3d':
            if self.ego:
                ...
                ys = torch.linspace(0.5, H - 0.5, H, dtype=dtype,
                                    device=device).view(1, H, 1).expand(num_points_in_pillar, H, W) / H
            ref_3d = torch.stack((xs, ys, zs), -1) #--z,h,w,3-->z,3,h*w-->z,h*w,3-->b,z,h*w,3
            #----debug-----------
        #     ref_y, ref_x, ref_z = torch.meshgrid(
        #     torch.linspace(0.5, H - 0.5,H, dtype=dtype, device=device),
        #     torch.linspace(0.5, W - 0.5, W, dtype=dtype, device=device),
        #     torch.linspace(0.5, Z - 0.5, 8, dtype=dtype, device=device),
        # )
        #     ref_y = ref_y / H
        #     ref_x = ref_x / W
        #     ref_z = ref_z / Z
    ...
    def point_sampling(self, reference_points, pc_range,  img_metas):
        ...
        #-------适配vod和tj4d--------
        if not isinstance(img_metas[0]['filename'], list) and img_metas[0]['filename'].split('/')[1] in ['TJ4DRadSet_4DRadar', 'vod']:
            lidar2img = lidar2img.unsqueeze(1)
        if self.ego:
            ego2lidar=img_metas[0]['ego2lidar']
            ego2lidar = reference_points.new_tensor(ego2lidar)

        #--torch.Size([1, 8, 9600, 3])        
        ...
        #----torch.Size([8, 1, 6, 9600, 4, 1])
        reference_points = reference_points.view(
            D, B, 1, num_query, 4).repeat(1, 1, num_cam, 1, 1).unsqueeze(-1)
        #---torch.Size([8, 1, 6, 9600, 4, 4])
        ...
        #----torch.Size([6, 1, 9600, 8, 2])，torch.Size([6, 1, 9600, 8])-----
    ...
    def forward(self,
        ...
        ref_3d = self.get_reference_points(
            bev_h, bev_w, bev_z, bev_z, dim='3d', bs=bev_query.size(1),  device=bev_query.device, dtype=bev_query.dtype) 
        #--torch.Size([1, 9600, 1, 2])
...
class OccupancyLayerV1(MyCustomBaseTransformerLayer):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(OccupancyLayerV1, self).__init__(
            attn_cfgs=attn_cfgs,
        ...
        for i in range(conv_num):
            ...
            deblock = nn.Sequential(conv_layer,
                                    build_norm_layer(norm_cfg_3dconv, embed_dims)[1],
    ...
    def forward(self,
        ...
        **kwargs contains some specific arguments of attentions.
        ...
        for layer in self.operation_order:
            ...
            if layer == 'conv':
                bs = query.shape[0]
                identity = query #---b,h,w,z,c-->b,c,z,h,w-->b,h,w,z,c
                query = query.reshape(bs, bev_h, bev_w, bev_z,-1).permute(0, 4, 3, 1, 2)
                for i in range(len(self.deblock)):
                    query = self.deblock[i](query)
                query = query.permute(0, 3, 4, 2, 1).reshape(bs, bev_h*bev_w*bev_z, -1)
