...
class RadarPoints(BasePoints):
    """ Radar Points of instances in LIDAR/ego coordinates.
    ...
    def flip(self, bev_direction="horizontal"):
        ...
        if bev_direction == "horizontal":
            self.tensor[:, 1] = -self.tensor[:, 1] #--y
            self.tensor[:, 4] = -self.tensor[:, 4] #--vy
            #self.tensor[:, 5] = -self.tensor[:, 5]
            #self.tensor[:, 7] = -self.tensor[:, 7]
            #self.tensor[:, 9] = -self.tensor[:, 9]
        elif bev_direction == "vertical":
            self.tensor[:, 0] = -self.tensor[:, 0] #--x
            self.tensor[:, 3] = -self.tensor[:, 3] #--vx
