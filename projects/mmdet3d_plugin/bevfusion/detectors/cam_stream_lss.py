...
class CamEncode(nn.Module):
    ...
    def get_depth_feat(self, x):
        # Depth
        x = self.depthnet(x) #--[6,105,112,200]

        depth = self.get_depth_dist(x[:, :self.D]) #--[6,41,112,200]
        new_x = depth.unsqueeze(1) * x[:, self.D:(self.D + self.C)].unsqueeze(2)#--对应元素相乘
        return depth, new_x

    def forward(self, x):
        depth, x = self.get_depth_feat(x) #--[6,64,41,112,200]
...
class LiftSplatShoot(nn.Module):
    def __init__(self, lss=False, final_dim=(900, 1600), camera_depth_range=[4.0, 45.0, 1.0], pc_range=[-50, -50, -5, 50, 50, 3], downsample=4, grid=3, inputC=256, camC=64):
        ...
        self.bx = bx#----tensor([-49.7500, -49.7500,  -4.7500], device='cuda:0')
        self.nx = nx #---tensor([200, 200,  16], device='cuda:0')
 
        self.downsample = downsample
        self.fH, self.fW = self.final_dim[0] // self.downsample, self.final_dim[1] // self.downsample
        self.camC = camC
        self.inputC = inputC
        self.frustum = self.create_frustum() #--#--[41,112,200,3],代表原始图像大小和深度
    ...
    def create_frustum(self):
        ...
        ogfH, ogfW = self.final_dim  #---[900,1600]
        fH, fW = self.fH, self.fW  #--[112,200]
        ds = torch.arange(*self.grid_conf['dbound'], dtype=torch.float).view(-1, 1, 1).expand(-1, fH, fW) #[41,112,200]
        D, _, _ = ds.shape
        xs = torch.linspace(0, ogfW - 1, fW, dtype=torch.float).view(1, 1, fW).expand(D, fH, fW)
        ys = torch.linspace(0, ogfH - 1, fH, dtype=torch.float).view(1, fH, 1).expand(D, fH, fW)

        # D x H x W x 3
        frustum = torch.stack((xs, ys, ds), -1) #--[41,112,200,3]
    ...
    def get_geometry(self, rots, trans, post_rots=None, post_trans=None,extra_rots=None,extra_trans=None):
        ...
        if post_rots is not None or post_trans is not None:
            ...
            points = self.frustum.repeat(B, N, 1, 1, 1, 1).unsqueeze(-1)  # B x N x D x H x W x 3 x 1 torch.Size([1, 6, 41, 112, 200, 3, 1])

        # cam_to_ego rots和trans已经是img2lidar了
        points = torch.cat((points[:, :, :, :, :, :2] * points[:, :, :, :, :, 2:3],
                            points[:, :, :, :, :, 2:3]
                            ), 5) #--torch.Size([1, 6, 41, 112, 200, 3, 1])
        points = rots.view(B, N, 1, 1, 1, 3, 3).matmul(points).squeeze(-1)
        points += trans.view(B, N, 1, 1, 1, 3) #--torch.Size([1, 6, 41, 112, 200, 3])
        #---转到lidar坐标系下
    ...
    def get_cam_feats(self, x):
        ...
        x = x.view(B * N, C, H, W)
        x, depth = self.camencode(x) #--torch.Size([6, 64, 41, 112, 200])，torch.Size([6, 41, 112, 200])
        x = x.view(B, N, self.camC, self.D, H, W) #--torch.Size([1,6, 64, 41, 112, 200])
        x = x.permute(0, 1, 3, 4, 5, 2) #--torch.Size([1,6,  41, 112, 200,64])
        depth = depth.view(B, N, self.D, H, W) #--torch.Size([1,6, 41, 112, 200])
        return x, depth

    def voxel_pooling(self, geom_feats, x):
        B, N, D, H, W, C = x.shape #--1,6,41,112,200,64
        Nprime = B * N * D * H * W #--5510400
        batch_size = x.shape[0]

        # flatten x
        x = x.reshape(Nprime, C)  #--torch.Size([5510400, 64])

        # flatten indices第几个格子，从-50，-50，-5开始算
        geom_feats = ((geom_feats - (self.bx.to(x.device) - self.dx.to(x.device) / 2.)) / self.dx.to(x.device)).long() #--1,6,41,112,200,3
        geom_feats = geom_feats.view(Nprime, 3)#--torch.Size([5510400, 3])
        batch_ix = torch.cat([torch.full([Nprime // B, 1], ix,
                                         device=x.device, dtype=torch.long) for ix in range(B)])
        batch_ix = batch_ix.to(geom_feats.device) #--torch.Size([5510400, 1])
        geom_feats = torch.cat((geom_feats, batch_ix), 1) #--torch.Size([5510400, 4])
        # filter out points that are outside box 取划定点云范围内的#--200,200,16
        kept = (geom_feats[:, 0] >= 0) & (geom_feats[:, 0] < self.nx.to(x.device)[0]) \
               & (geom_feats[:, 1] >= 0) & (geom_feats[:, 1] < self.nx.to(x.device)[1]) \
               & (geom_feats[:, 2] >= 0) & (geom_feats[:, 2] < self.nx.to(x.device)[2])
        x = x[kept] #--2989223,64
        geom_feats = geom_feats[kept] #--2989223,4
        # get tensors from the same voxel next to each other
        ranks = geom_feats[:, 0] * (self.nx.to(x.device)[1] * self.nx.to(x.device)[2] * B) \
                + geom_feats[:, 1] * (self.nx.to(x.device)[2] * B) \
                + geom_feats[:, 2] * B \
                + geom_feats[:, 3]
        sorts = ranks.argsort() #--2989223# 给每一个点一个rank值，rank相等的点在同一个batch，并且在在同一个格子里面
        x, geom_feats, ranks = x[sorts], geom_feats[sorts], ranks[sorts]
        # cumsum trick
        if not self.use_quickcumsum:
            x, geom_feats = cumsum_trick(x, geom_feats, ranks)
        else: #--275621,,64    275621,4
            x, geom_feats = QuickCumsum.apply(x, geom_feats, ranks)

        # griddify (B x C x Z x X x Y) 1,64,16,200,200
        final = torch.zeros((B, C, self.nx.to(x.device)[2], self.nx.to(x.device)[0], self.nx.to(x.device)[1]), device=x.device)
        final[geom_feats[:, 3], :, geom_feats[:, 2], geom_feats[:, 0], geom_feats[:, 1]] = x

        return final

    def get_voxels(self, x, rots=None, trans=None, post_rots=None, post_trans=None,extra_rots=None,extra_trans=None):
        geom = self.get_geometry(rots, trans, post_rots, post_trans,extra_rots,extra_trans)#--torch.Size([1, 6, 41, 112, 200, 3])
        x, depth = self.get_cam_feats(x) #--torch.Size([1,6,  41, 112, 200,64])#--torch.Size([1,6, 41, 112, 200])
        x = self.voxel_pooling(geom, x) #---[1,64,16,200,200]
        return x, depth

    def s2c(self, x):
        B, C, H, W, L = x.shape #---W是x，L是y
        bev = torch.reshape(x, (B, C*H, W, L)) #--直接reshape掉高度维
        bev = bev.permute((0,1,3,2)) #（1,1024,224,288），BEV左下角是原点
        return bev
    ...
    def forward(self, x, rots, trans, lidar2img_rt=None, img_metas=None, post_rots=None, post_trans=None, extra_rots=None,extra_trans=None):
        x, depth = self.get_voxels(x, rots, trans, post_rots, post_trans,extra_rots,extra_trans) # [B, C, H, W, L][B,64,16,288,224]
        bev = self.s2c(x)
