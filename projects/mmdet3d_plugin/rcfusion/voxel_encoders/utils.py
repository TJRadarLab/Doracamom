...
class PFNLayer_Radar(nn.Module):
    ...
    def __init__(self,
        ...
        # self.linear4 = nn.Linear(out_channels, out_channels, bias=False)
        assert mode in ['max', 'avg']
        self.mode = mode
    ...
    def forward(self, inputs, num_voxels=None, aligned_distance=None):
        ...
        Returns:
            torch.Tensor: Features of Pillars.
        """
        spatio_input = inputs.index_select(2,torch.tensor([0,1,2,7,8,9,10,11]).to(inputs.device))
        velocity_input = inputs.index_select(2,torch.tensor([3,4,12,13]).to(inputs.device))
...
class PFNLayer_RadarV2(nn.Module):
    """Pillar Feature Net Layer.
    ...
    def forward(self, inputs, num_voxels=None, aligned_distance=None):
        ...
        Returns:(xyz v_xv_y snr XcYcZcXpYp Vcx Vcy SNRc)[X,10,14]
            torch.Tensor: Features of Pillars.
        """
        spatio_input = inputs.index_select(2,torch.tensor([0,1,2,6,7,8,9,10]).to(inputs.device))
        velocity_input = inputs.index_select(2,torch.tensor([3,4,11,12]).to(inputs.device))
...
class PFNLayer_Radar_vod(nn.Module):
    """Pillar Feature Net Layer.
