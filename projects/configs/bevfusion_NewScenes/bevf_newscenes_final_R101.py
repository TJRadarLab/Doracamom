...
model = dict(
    ...
    lc_fusion=True,
    camera_stream=True, 
    lss=False,  #-------不采用原始LSS中的r18encoder
    grid=0.5, 
    num_views=6,
    final_dim=final_dim, #--(1080, 1920)
    pc_range=point_cloud_range, 
    downsample=downsample,  #--8
    camera_depth_range=[1,60,1],#----0.5/1
    img_depth_loss_method='kld',
    img_depth_loss_weight=1.0,
    pts_voxel_layer=dict(
        max_num_points=10, #----这里设置成10
        point_cloud_range=point_cloud_range,
        voxel_size=voxel_size,
        max_voxels=(30000, 40000)),

    pts_voxel_encoder=dict(
            type='PillarFeatureNetV1',
            in_channels=8,
            feat_channels=[64],
            with_distance=False,
            voxel_size=voxel_size,
    ...
    pts_middle_encoder=dict(
        type='PointPillarsScatter', in_channels=64, output_shape=[320, 480]), #---这里修改成[320, 480]
    pts_backbone=dict(
        type='SECOND',
        in_channels=64,
    ...
    img_backbone=dict(
        ...
        frozen_stages=1,
        norm_cfg=dict(type='SyncBN', requires_grad=False), #----改成syncBN
        norm_eval=True,
        style='caffe',
    ...
    dict(
        ...
        transforms=[
            ...
            dict(
                ...
                with_label=False),
            dict(type='CustomCollect3D', keys=['img','points']) #---加入radar points
        ])
]
...
evaluation = dict(interval=4, pipeline=test_pipeline) #----eval评估间隔，改24

# optimizer = dict(type='AdamW', lr=0.001, weight_decay=0.05) #---默认0.001
...
log_level = 'INFO'
work_dir = 'work_dirs/NewScenes_Final/bevf_newscenesfinal_fusion_R101'

load_lift_from = 'work_dirs/NewScenes_Final/bevf_LSS_depth_newscenesfinal_cam_bevpoolv2_syncBN_R101/epoch_24.pth'
resume_from = None
load_from = 'work_dirs/NewScenes_Final/pointpillars_newscenes_4DRadar_pillarVFEDEBUG/epoch_24.pth'
workflow = [('train', 1)]
